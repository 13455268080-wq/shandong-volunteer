...
// ---- 数据加载 ----
const DATA_PARTS = ['data/rec2025_p0.json','data/rec2025_p1.json','data/rec2025_p2.json','data/rec2025_p3.json','data/rec2025_p4.json','data/rec2025_p5.json','data/rec2025_p6.json','data/rec2025_p7.json','data/rec2025_p8.json','data/rec2025_p9.json'];

const H = ['school','major','batch','subject','plan','score','rank','is985','is211'];
 ...
 * 核心：生成96志愿推荐
 */
function generateRecommendations(rank, score, subjects, batchFilter, keyword, majorPreference) {
  if (!mainData || mainData.length === 0) {
    return { items: [], summary: { total: 0, chong: 0, wen: 0, bao: 0, selected: 0 } };
  }

  const isMilitaryPref = majorPreference === 'military';
  ...
  for (const raw of mainData) {
    ...
    if (isMilitaryPref) {
      ...
      if (majorPreference === 'science') {
        const mt = classifyMajor(rec.major);
        if (mt === 'arts') continue;
      }
      if (majorPreference === 'arts') {
        const mt = classifyMajor(rec.major);
        if (mt === 'science') continue;
      }
    }
  ...
  const chongPick = pickAndDedup(chongPool, 19, excludeSet);
  chongPick.forEach(i => excludeSet.add(i.school));
  const wenPick = pickAndDedup(wenPool, 48, excludeSet);
  wenPick.forEach(i => excludeSet.add(i.school));
  const remainingSlots = 96 - chongPick.length - wenPick.length;
  const baoPick = pickAndDedup(baoPool, remainingSlots, excludeSet);

  let finalItems = [...chongPick, ...wenPick, ...baoPick];
  if (finalItems.length < 96) {
    const usedSchools = new Set(finalItems.map(i => i.school));
    for (const item of [...chongPool, ...wenPool, ...baoPool]) {
      if (finalItems.length >= 96) break;
...
function handleQuery() {
  ...
  const majorPreference = document.getElementById('majorPreference').value;
  ...
  const result = generateRecommendations(rank, score, subjects, batch, keyword, majorPreference);
  _lastResult = result;
  _lastRank = rank;
  _lastScore = score;
...
function renderResults(result, rank, score) {
  ...
  const chongItems = result.items.filter(i => i.displayTier === '冲');
  const wenItems = result.items.filter(i => i.displayTier === '稳');
  const baoItems = result.items.filter(i => i.displayTier === '保');
  ...
  const renderItem = (item) => {
    ...
    const majorText = hasChildren
      ? item.children.slice(0, 3).map(c => c.major).join('、') + (item.children.length > 3 ? ' 等' + item.children.length + '个专业' : '')
      : (item.major || '');
    ...
    if (hasChildren) {
      ...
      item.children.forEach((c, i) => {
        html += [
          ...
          '<div class="child-major">' + (i + 1) + '. ' + c.major + (c.subject && c.subject !== '不限' ? ' <span class="tag">' + c.subject + '</span>' : '') + '</div>',
...
function resetForm() {
  ...
  document.getElementById('majorPreference').value = '';
